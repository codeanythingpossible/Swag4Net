...
let parseInlineOrReference f node : ParsingState<'T InlineOrReference> =
  parsing {
    let! r =
        match node with
...
let parseOptionalInlineOrReference f node  : ParsingState<'T InlineOrReference option> =
  match node with
        ...
        | SObject props ->
            ...
            parsing {
                ...
                parsing {
                  ...
                  | Some (SObject props) -> 
                      let! r =
                        ...
                        |> List.map (
                            fun (name, v) -> 
                              let schema = parseInlineOrReference parseSchema v
              ...
              let parseInheritance name =
                node |> selectToken name |> parseOptionalInlineOrReference parseSchema

              let! items =
                node |> selectToken "items" |> parseOptionalInlineOrReference parseSchema
...
let parseSchemaRef node : ParsingState<Schema InlineOrReference option> =
  parsing {
    let! r = 
      node
      |> selectToken "schema"
      |> parseOptionalInlineOrReference parseSchema
    return r
  }

let parseParameter node : ParsingState<Parameter InlineOrReference> =
  parsing {
                  ...
                  |> ParsingState.map (
                        fun s -> 
                          ...
                          | Some o ->
                              let d = 
                                { Schema = o
                                  Examples = Map.empty //Map<string, Example InlineOrReference>
  ...
  parsing {
    ...
    | Some(SObject props) -> 
        ...
        let! def = 
          ...
          |> parseOptionalInlineOrReference parseResponse
        ...
        let! responsesDefinitions = 
          ...
          |> List.map (
               fun (code, n) -> 
                (parseOptionalInlineOrReference parseResponse) (Some n) |> ParsingState.map (fun r -> code,r)
             )
